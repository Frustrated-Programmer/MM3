...
let cards = [
	...
	{//#28 - sabotage
		...
		effect: function(user, target) {
			target.bonusDefense--;
			target.send(`You've been sabotaged, and left vulnerable to attack! -1 🛡 Defense!`);
		}
	},
	{//#29 - unstoppable
		name: `unstoppable`,
	...
	{//#31 - oath
		...
		effect: function(user, target) {
			target.bonusDefense--;
			target.send(`A blood oath awakens! -1 🛡 Defense!`);
			user.bonusDefense--;
			user.send(`A blood oath awakens! -1 🛡 Defense`);
		}
	},
	{//#32 - reveal
		name: `reveal`,
	...
	{//#44 - fortify
		...
		effect: function(user, target) {
			target.minDef(1);
			target.send(`You've been fortified! 1 🛡 Minimum Defense`);
			for(let i = 0; i < target.visitors.length; i++) {
				target.visitors[i].minDef(1);
				if(target.visitors[i] !== user) {
					target.visitors[i].send(`You've been fortified! 1 🛡 Minimum Defense`);
				}
			}
		}
	},
	...
	{//#53 - love
		...
		effect: function(user, target) {
			target.bonusDefense--;
			target.send(`A loving bond awakens! +1 🛡 Defense!`);
			user.bonusDefense--;
			user.send(`A loving bond awakens! -1 🛡 Defense`);
		}
	},
	{//#54 - shuffle
		name: `shuffle`,
	...
	{//leave
		...
		effect: function(message, args) {
			...
			switch(success) {
				...
				default:
					message.channel.send(`Something went wrong - the issue has been reported.`);
					console.log(`${yourID} -  .leave failure - ${yourParty}`);
					break;
			}
...
client.on(`message`, function(message) {
	if(message.content.toLowerCase() === `mmprefix`) {
		message.channel.send(`Your prefix is \`${getPrefix(message.channel)}\` - use \`${getPrefix(message.channel)}help\` for more info.`);
		return;
	}
